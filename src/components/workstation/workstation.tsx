      ...
      <motion.div className="workstation-flight" animate={closing && !reduce ? { x: [0, -65, 150, 40, 420, "125vw"], y: [0, -70, 95, -125, -25, "18vh"], rotate: [0, -8, 18, -22, 9, 32], scale: [1, .82, .68, .55, .42, .28] } : undefined} transition={{ duration: reduce ? .1 : 1.32, times: [0,.16,.34,.54,.73,1], ease: "easeInOut" }}>
        ...
        <div className="workstation">
          ...
          <section className="viewer-panel" ref={panelRef}>
            ...
            <AnimatePresence>{beam && <motion.svg key={beam.id} className="search-flight" viewBox={`0 0 ${Math.max(1, panelRef.current?.clientWidth ?? 1)} ${Math.max(1, panelRef.current?.clientHeight ?? 1)}`} preserveAspectRatio="none" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><defs><filter id="blue-glow"><feGaussianBlur stdDeviation="4" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs><motion.path d={`M ${beam.x1} ${beam.y1} C ${beam.x1 - 120} ${beam.y1 + 45}, ${beam.x2 + 120} ${beam.y2 - 45}, ${beam.x2} ${beam.y2}`} fill="none" stroke="url(#search-gradient)" strokeWidth="3" filter="url(#blue-glow)" initial={{ pathLength: 0, opacity: 0 }} animate={{ pathLength: [0, 1, 1, 1], opacity: [0, 1, 1, 0] }} transition={{ duration: 1.05, times: [0,.34,.82,1] }}/><linearGradient id="search-gradient"><stop stopColor="#f4fbff"/><stop offset=".45" stopColor="#78bde9"/><stop offset="1" stopColor="#3277a8"/></linearGradient></motion.svg>}</AnimatePresence>
